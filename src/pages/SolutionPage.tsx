...
const SolutionPage = () => {
    ...
    }, [textbookId, chapter, section, problem]);
    ...
    const handlePostSolutionClicked = () => {
        navigate(`/create/${textbookId}`, { state: { chapter, section, problem } });
    }
    ...
    return (
        <div className='solutionsContainer'>
            <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', width: '50%' }}>
                    <div style={{ display: 'flex', flexDirection: 'row', width: '100%', height: '100%', alignItems: 'flex-end', justifyContent: 'flex-end' }}>
                        <button
                            className='rateSolutionButton'
                            onClick={handlePostSolutionClicked}
                        >
                            <AddIcon />&nbsp;Post Solution
                        </button>
                        <button
                            className='newSolutionButton'
                            onClick={handleNewSolutionClicked}
                        >
                            <ListIcon />&nbsp;&nbsp;Select New Solution
                        </button>
                    </div>
                </div>
            </div>
            { /** TODO: sort the solutions by rating */}
            {isLoading ? <Loading /> : solutions.length === 0 ? <NoSolutionsFound /> : solutions.map((solution, i) => (
                <div className='singleSolutionContainer' key={i}>
                    <div className='solutionTitleContainer'>
                        <div className='solutionUserContainer' style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', width: '100%', paddingLeft: '30px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center', height: '64px', marginLeft: '10px' }}>
                                <span className='userNameText'>{solution.userName}</span>
                                <span className='solnCreatedText'>Solution Created: {formatEpochToDate(solution.timeCreated)}</span>
                            </div>
                        </div>
...
const formatEpochToDate = (secondsSinceEpoch: number) => {
    const date = new Date(secondsSinceEpoch);
...
const NoSolutionsFound: React.FC = () => {
    return (
        <div style={{ height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#e5e5e5', width: "100%", borderRadius: "15px" }}>
            <h1>No Solutions Found</h1>
        </div>
    )
}
